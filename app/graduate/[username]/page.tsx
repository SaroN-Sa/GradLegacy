...
import GraduateTabs, { GraduateTab } from "@/components/graduate/GraduateTabs";
...
export default function GraduatePage() {
  ...
  const [activeTab, setActiveTab] = useState<GraduateTab>("wishes");
      ...
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-[#0b1120] via-[#0f172a] to-[#162035] px-4">
        <div className="w-full max-w-md rounded-3xl bg-white p-10 text-center shadow-2xl">
          <div className="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-2xl bg-[#0f172a]">
            <GraduationCap className="text-[#FFD700]" />
          </div>
          <h1 className="text-2xl font-bold">Page Not Found</h1>
          <p className="mt-3 text-gray-500">This graduation page does not exist or has not been approved.</p>
          <a href="/" className="mt-8 inline-flex rounded-xl bg-[#0f172a] px-6 py-3 text-white">
            Go Home
          </a>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0b1120] via-[#0f172a] to-[#162035]">
      {/* Ambient Background */}
      <div className="pointer-events-none fixed right-0 top-0 h-[28rem] w-[28rem] rounded-full bg-blue-500/10 blur-[120px]" />
      <div className="pointer-events-none fixed bottom-0 left-0 h-[32rem] w-[32rem] rounded-full bg-[#FFD700]/[0.06] blur-[120px]" />

      {/* Graduate Header */}
      <GraduateHeader profile={profile} copied={copied} onShare={handleShare} />
      ...
      <div className="relative z-10 mx-auto max-w-3xl px-4 pb-16 sm:px-6">
        ...
        <div className="mt-5">
          {activeTab === "wishes" && (
            <WishesSection userId={profile.userId} graduateName={profile.fullName} />
          )}

          {activeTab === "gallery" && (
            <GallerySection userId={profile.userId} graduateName={profile.fullName} />
          )}

          {activeTab === "timeline" && (
            <TimelineSection userId={profile.userId} graduateName={profile.fullName} />
          )}
        </div>
